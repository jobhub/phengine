...
class String {
	...
	public static function substr( $str, $from = 0, $count = 0 ) {
		if ( $from < 0 ) {
			$from = self::strlen( $str ) + $from;
		}
		if ( $count < 0 ) {
			$str   = mb_substr( $str, 0, self::strlen( ( $str ) + $count, 'UTF-8' ) );
			$count = 0;
		}
		if ( 0 == $count ) {
			$count = self::strlen( $str ) - $from;
		}
	...
	public static function quoteIdentifier( $ident, $alias = null, $as = ' AS ' ) {
		...
		if ( is_array( $ident ) ) {
			...
			foreach ( $ident as $segment ) {
				$segments[] = self::_quoteIdentifier( $segment );
			...
			$quoted = self::_quoteIdentifier( $ident );
		}

		if ( $alias !== null ) {
			$quoted .= $as . self::_quoteIdentifier( $alias );
		}

		return $quoted;
	}

	public static function _quoteIdentifier ($value) {
		$q = '"';
		return ($q . str_replace("$q", "$q$q", $value) . $q);
	}

	public static function mb_trim($string, $charlist='\\\\s\\\\pZ\\\\pM\\\\pC', $ltrim=true, $rtrim=true)
	{
	  if (empty($string)) {
	    return '';
	  }
	...
	public static function generateUUID($entropy_pool = false) {
	...
	public static function generateAccount($id) {
	  $mod1=11;
	  $mod2=10;
	  $id = intval($id);
