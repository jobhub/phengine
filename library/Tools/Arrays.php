...
class Arrays {
	 ...
	 * @return array
	 */
	static function assocToArray( $array, $key = null, $value = null ) {
		$result = [ ];
		foreach ( $array as $k => $v ) {
	...
	/**
	 * Генерируются все возможные варианты из многомерного массива, массив должен иметь квадратную структуру.
	 *
	 * @param array $arrays
	...
	public static function createSortLimitFromPost ( $q=false, $for_mongo = false ) {
		if (false===$q) {
	    $q = $_POST;
	  }
	  if (isset($q['limit'])) {
	    $limit = intval($q['limit']);
	  } elseif (isset($q['count'])) {
	    $limit = intval($q['count']);
	  } else {
	    $limit = false;
	  }
	  if (isset($q['offset'])) {
	    $offset = $q['offset'];
	  } elseif (isset($q['page']) && $limit) {
	    $offset = (intval($q['page'])-1)*intval($limit);
	    if ($offset<0) {
	      $offset = 0;
	    }
	  } else {
	  ...
	  if (isset($q['dir']) && 'desc'==strtolower($q['dir'])) {
	    $dir = 'DESC';
	  } else {
	    $dir = 'ASC';
	  }
	  ...
	  if (isset($sort) && !empty ($sort)) {
	    foreach ($sort as $s) {
	      ...
	      $tnulls = $nulls;
	      if (is_array($s)) {
	        if (isset($s['direction'])) {
	          $tdir = ('desc'==strtolower($s['direction'])) ? 'DESC' : 'ASC';
	        }
	        if (!isset($s['property'])) {
	          continue;
	        }
	        $prop = String::mb_trim($s['property']);
	        if (isset($s['nulls'])) {
	          $tnulls = ('last'==strtolower($s['nulls']))?'NULLS LAST':'';
	        }
	      }
	      ...
	      if(!$for_mongo) {
	        $prop = String::_quoteIdentifier($prop);
	        $order[] = new \Engine\Db\Expr("$prop $tdir $tnulls");
	      } else {
	        $order[] = array('property'=>$prop, 'direction'=>$tdir);
	      }
	    }
	  }
