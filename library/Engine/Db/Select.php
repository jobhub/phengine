...
use Engine\Db\Select\Exception, Engine\Db\Expr;

/**
 * Zend Framework Port of Zend_Db_Select
 *
 */
...
class Select
{
     ...
     * Class constructor
     *
     * @param \Phalcon\Db\Adapter $adapter
     */
    public function __construct(\Phalcon\Db\Adapter $adapter)
    {
     ...
     * @return Select This Engine\Db\Select object.
     */
     ...
     * table.
     *
     * The second parameter can be a single string or Engine\Db\Expr object,
     * or else an array of strings or Engine\Db\Expr objects.
     *
     ...
     *
     * The first parameter has to be an array of Engine\Db\Select or
     * sql query strings.
     *
     * <code>
     * $sql1 = $db->select();
     ...
     * particular Engine\Db\Select object.
     *
     * @return Adapter\Pdo\Postgresql
     */
    ...
    protected function _renderLimitoffset($sql)
    {
         ...
         */
        if ($count > 0) {
            $sql = trim($this->_adapter->limit($sql, $count));
        }
				if ($offset > 0) {
					$sql .= " OFFSET {$offset}";
				}
