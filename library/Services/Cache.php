...
class Cache extends AbstractService {
	 ...
	 */
	public function clearCache( $keys = array(), $folders = array() ) {
		$di = $this->getDi();
		...
		if ( empty( $keys ) && $this->_config->application->debug ) {
			$keys = $default_keys;
		}
		...
		foreach ( $keys as $k ) {
			...
			$q_keys = $cache->queryKeys();

			foreach ( $q_keys as $q_key ) {
				if(in_array($q_key, $keys)) {
					$cache->delete( $q_key );
				}
			}
		}
		...
		if ( empty( $folders ) && $this->_config->application->debug ) {
			$folders = $default_folders;
		}

		// clear files cache
		if ( in_array( 'data', $folders ) ) {
			$files = glob( $this->_config->application->cache->output->cacheDir . '*' ); // get all file names
		...
		if ( in_array( 'annotations', $folders ) ) {
			if ( $this->_config->annotations && $this->_config->annotations->annotationsDir ) {
				$files = glob( $this->_config->annotations->annotationsDir . '*' ); // get all file names
				foreach ( $files as $file ) { // iterate files
					if ( is_file( $file ) ) {
						@unlink( $file ); // delete file
					}
